...
export class ECSService {
    async registerTaskDefinition(input) {
        const containerName = `df-project-${input.project_id}`;
        const family = `df-project-${input.project_id}`;
        ...
        const response = await ecsClient.send(cammand);
        const taskDefinitionArn = response.taskDefinition
            ?.taskDefinitionArn;
        if (!taskDefinitionArn) {
            throw new Error("AWS did not return task definition ARN");
        }
        return taskDefinitionArn;
    }
    async createService(input, TargetGroupArn, taskDefinitionArn) {
        const subnetIds = process.env.AWS_SUBNET_ID
            ?.split(",")
            .map(id => id.trim());
        const securityGroupId = process.env.AWS_FARGATE_SECURITY_GROUP_ID;
        ...
        const serviceName = `df-project-${input.project_id}-service`;
        const containerName = `df-project-${input.project_id}`;
