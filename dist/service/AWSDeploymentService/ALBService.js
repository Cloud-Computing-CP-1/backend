...
import { ElasticLoadBalancingV2Client, CreateTargetGroupCommand, CreateRuleCommand } from "@aws-sdk/client-elastic-load-balancing-v2";
export class ALBService {
    async createTargetGroup(input) {
        const vpcId = process.env.AWS_VPC_ID;
        if (!vpcId) {
            throw new Error("AWS_VPC_ID is not configured");
        }
        const targetGroupName = `df-project-${input.project_id}-tg`;
        ...
        if (!targetGroupArn) {
            throw new Error("AWS did not return Target Group ARN");
        }
        return targetGroupArn;
    }
    async createListenerRule(projectId, projectSlug, targetGroupArn) {
        ...
        if (!ruleArn) {
            throw new Error("AWS did not return Listener Rule ARN");
        }
...
//# sourceMappingURL=ALBService.js.map