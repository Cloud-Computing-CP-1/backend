...
export class GithubService {
    static _webhook_register_service = async (AcessToken, owner, repo) => {
        const webhookURL = `${process.env.BACKEND_URI}/api/auth/github/webhook`;
        const octokit = new Octokit({
            auth: AcessToken
        });
        const hooks = await octokit.request("GET /repos/{owner}/{repo}/hooks", {
            owner,
            repo,
            headers: {
    ...
    static getwebhook = async (accessToken, owner, repo) => {
        const octokit = new Octokit({
            auth: accessToken
        });
